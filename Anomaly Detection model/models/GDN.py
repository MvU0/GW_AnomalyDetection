...
class GNNLayer(nn.Module):
    def __init__(self, in_channel, out_channel, heads=1, node_num=100):
...
class GDN(nn.Module):
    def __init__(self, edge_index_sets, node_num, dim=64, out_layer_inter_dim=256, input_dim=10, out_layer_num=1, topk=20):
        ...
        embed_dim = dim
        self.embedding = nn.Embedding(node_num, embed_dim)   # EMBEDDING
        ...
        print('self.gnn_layers:', self.gnn_layers)
    ...
    def init_params(self):
        nn.init.kaiming_uniform_(self.embedding.weight, a=math.sqrt(5)) # Initialise embedding weights
    ...
    def forward(self, data, org_edge_index):
        ...
        for i, edge_index in enumerate(edge_index_sets):
            ...
            all_embeddings = self.embedding(torch.arange(node_num).to(device)) # EMBEDDING OF NODES
            ...
            self.learned_graph = topk_indices_ji  # Here the graph is stored (or better said adjacancy matrix) after only considering top k edges
        ...
        learned_graph = self.learned_graph.clone()
        return out.float().to(device), learned_graph
